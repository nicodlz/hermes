    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-secondary text-sm sm:text-base">Message and proposal templates</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          {isLoading ? (
            <div className="bg-card rounded-lg border border-border p-8 text-center text-secondary shadow-card">
              Loading...
            </div>
              ...
              return (
                ...
                >
                  <div className="px-4 py-3 border-b border-border flex items-center gap-2">
                    ...
                    <span className="font-semibold text-foreground">{type.replace(/_/g, " ")}</span>
                    <span className="text-sm text-secondary">({temps.length})</span>
                  </div>
          ...
          {selectedTemplate ? (
            <div className="bg-card rounded-lg border border-border shadow-card">
              <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                ...
                <span className="text-sm text-secondary">{selectedTemplate.name}</span>
              </div>
              
              {/* Variables */}
              {selectedTemplate.variables && (
                <div className="px-4 py-3 border-b border-border bg-nested">
                  <p className="text-xs text-secondary mb-2">Variables:</p>
                  <div className="flex flex-wrap gap-2">
                    {JSON.parse(selectedTemplate.variables).map((v: string) => (
                      <input
                        ...
                        className="px-2 py-1 text-sm rounded border border-border bg-card text-foreground placeholder:text-muted-foreground"
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="p-4">
                ...
                <div className="prose prose-sm dark:prose-invert max-w-none">
                  <pre className="whitespace-pre-wrap font-sans text-sm bg-nested text-slate-800 dark:text-slate-200 p-4 rounded-lg">
            ...
            <div className="bg-nested rounded-lg border border-dashed border-slate-300 dark:border-border p-8 text-center text-secondary">
